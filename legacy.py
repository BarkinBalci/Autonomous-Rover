...
from ESC_Control import leftMotorSpeed, rightMotorSpeed
from Ultrasonic_sensors import Ultrasonic_Readout, COLLISION
    ...
    def startLoop(self):
        ...
        for stream in stream_names:
            if stream in ["disparity", "disparity_color", "depth"]:
                ...
                trackbar_name = 'Disparity'
                conf_thr_slider_min = 0
                conf_thr_slider_max = 255
                conf_thr_slider_default = 240
                cv2.createTrackbar(trackbar_name, wls_stream, conf_thr_slider_min, conf_thr_slider_max,self.on_trackbar_change)
                cv2.setTrackbarPos(trackbar_name, wls_stream, conf_thr_slider_default)
                cv2.createTrackbar(_lambda_trackbar_name, wls_stream, _lambda_slider_min, _lambda_slider_max, self.on_trackbar_change_lambda)
                cv2.setTrackbarPos(_lambda_trackbar_name, wls_stream, _lambda_slider_default)
                cv2.createTrackbar(_sigma_trackbar_name, wls_stream, _sigma_slider_min, _sigma_slider_max, self.on_trackbar_change_sigma)
        ...
        while self.runThread:
            ...
            if prev_right is not None:
                if prev_disp is not None:
                    ...
                    numPixels = np.prod(colored_wls.shape[:2])
                    (b, g, r) = cv2.split(colored_wls)
                    ...
                    if percentageR < 0.08:
                        WLS_Sensor = True
                        print("Possible Frontal Collision")
                    else:
                        WLS_Sensor = False

                    Ultrasonic_Readout()

#Collision Avoidance - Calibration Needed
                    if COLLISION[0] and COLLISION[1] and COLLISION[2] == True:
                        Left_Sensors = True
                    else:
                        Left_Sensors = False

                    if COLLISION[3] and COLLISION[4] and COLLISION[5] == True:
                        Right_Sensors = True
                    else:
                        Right_Sensors = False
                    ...
                    if WLS_Sensor == False:
                        leftMotorSpeed(5)
                        rightMotorSpeed(5)
                    elif WLS_Sensor == True and Left_Sensors == False:
                        leftMotorSpeed(-5)
                        rightMotorSpeed(5)
                    elif WLS_Sensor == True and Right_Sensors == False:
                        leftMotorSpeed(5)
                        rightMotorSpeed(-5)
                    elif WLS_Sensor and Right_Sensors and Left_Sensors == True:
                        leftMotorSpeed(-5)
                        rightMotorSpeed(-5)
                    ...
                    fig.canvas.draw()
                    cv2.imshow(wls_stream, colored_wls)
            ...
            for packet in self.data_packets:
                ...
                if packetData is None:
                    ...
                    cv2.putText(frame, "fps: " + str(frame_count_prev[window_name]), (25, 50), cv2.FONT_HERSHEY_SIMPLEX,
                                1.0, (0, 0, 0))
                    cv2.putText(frame, "NN fps: " + str(frame_count_prev['nn'][camera]), (2, frame.shape[0] - 4),
                    ...
                    cv2.putText(frame_bgr, "fps: " + str(frame_count_prev[window_name]), (25, 50),
                    ...
                    if args['draw_bb_depth']:
                        ...
                        if nnet_prev["entries_prev"][camera] is not None:
                            frame_bgr = show_nn(nnet_prev["entries_prev"][camera], frame_bgr, NN_json=NN_json,
                    ...
                    if len(frame.shape) == 2:
                        if frame.dtype == np.uint8:  # grayscale
                            ...
                            cv2.putText(frame, "fps: " + str(frame_count_prev[window_name]), (25, 50),
                                        cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255))
                        else:  # uint16
                            if args[
                                'pointcloud'] and "depth" in stream_names and "rectified_right" in stream_names and right_rectified is not None:
                                try:
                    ...
                    cv2.putText(bgr, "fps: " + str(frame_count_prev[window_name]), (25, 50), cv2.FONT_HERSHEY_SIMPLEX,
                                1.0, (0, 0, 0))
            ...
            if key == ord('w') and colored_wls is not None:
                filepath = '/home/barkin/Desktop/WLS-Images/'
                filename = ('current_label')
                current_time = datetime.datetime.now()
                fileformat = '.png'
                cv2.imwrite(filepath + filename + str(current_time) + fileformat, bgr)
            else:
